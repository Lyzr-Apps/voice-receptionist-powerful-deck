...
| `/api/rag` | GET/POST/PATCH/DELETE | RAG knowledge base operations |
| `/api/scheduler` | GET/POST/DELETE | Schedule management operations |
| `/api/activity-stream` | GET (SSE) | Real-time activity stream for manager-subagent patterns |
| `/api/lyzr-config` | GET | Get API key (utility — activity stream uses SSE proxy instead) |
...
## Activity Stream (Manager-SubAgent Pattern)

Uses a **server-side SSE proxy** at `/api/activity-stream` — API key never reaches the browser.
...
// Using hook
const { events, isConnected, activeAgent, status, connect, disconnect, reset } = useAgentActivityStream()

// Connect before calling agent (secure — uses server-side SSE proxy)
...
<ActivityPanel events={events} activeAgent={activeAgent} status={status} isConnected={isConnected} />
```
